...
app.use((_, res, next) => {
  ...
  res.setHeader(
    ...
    `default-src 'self'; base-uri 'self'; script-src 'self' 'nonce-${nonce}' 'strict-dynamic' https://challenges.cloudflare.com 'unsafe-eval'; frame-src https://challenges.cloudflare.com; img-src 'self' data:; style-src 'self' 'nonce-${nonce}';`
  );
  next();
});
