...
app.use((_, res, next) => {
  ...
  res.setHeader(
    ...
    `script-src 'self' 'nonce-${nonce}' https://challenges.cloudflare.com 'unsafe-eval' 'unsafe-inline'; frame-src https://challenges.cloudflare.com; img-src 'self' data:; style-src 'self' 'nonce-${nonce}';`
  );
  next();
});
